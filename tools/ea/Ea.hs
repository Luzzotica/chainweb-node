...
mkPayload (Genesis v tag cid c k a ns) = do
    ...
    genPayloadModule (v & versionConfig .~ defaultDevVersionConfig) (tag <> sshow cid) txs
  where
...
gen20ChainPayloads = traverse_ mk20ChainPayload [developmentKAD, mainnetKAD]
  where
    mk20ChainPayload (Genesis v tag cid c k a ns) = do
      ...
      genPayloadModule' (v & versionConfig .~ defaultDevVersionConfig) (tag <> sshow cid) cwTxs
