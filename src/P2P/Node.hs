...
guardPeerDb v nid peerDb pinf = do
  ...
  where
    isReserved :: Bool
    isReserved = _versionAllowReservedPeers v && isReservedHostAddress (_peerAddr pinf)
...
syncFromPeer node info = do
  ...
  where
    ...
    sync = do
        !p <- peerGetClient (_versionName v) nid Nothing Nothing
        liftIO $ logg node Debug $ "got " <> sshow (_pageLimit p) <> " peers " <> showInfo info
        void $ peerPutClient (_versionName v) nid (_p2pNodePeerInfo node)
